...
|---|---|---|
| **"App not installed"** · **"package conflicts with an existing package"** · **"App not installed as package conflicts"** — including when AFOS **isn't visibly installed at all** | All three are the same thing: **a signature mismatch.** A different build of AFOS — a debug copy, a profile build, or one signed with another key — is still on the phone somewhere, and Android refuses to replace an app with one signed differently. This is the protection working, not a broken download. A copy with no home-screen icon still counts: check **Secure Folder / Work profile / Second Space** if your phone has one, and **Settings → Apps → see all apps** (not just the ones with a launcher icon) for a leftover AFOS entry. | Uninstall every copy you find, then install again. *You'll be signed out and need to log in once; nothing on the server is lost — results, hall room, courses and everything else come straight back.* Every later update then installs in place. |
